...
try:
    ...
    st.error("Secrets-ல் Supabase விவரங்கள் இல்லை!")
    st.stop()
...
# --- தாவல்கள் (Tabs) உருவாக்குதல் ---
tab1, tab2 = st.tabs(["📝 தனித்தனியாகச் சேர்க்க", "📤 மொத்தமாகப் பதிவேற்ற (Bulk)"])
...
with tab1:
    with st.form("single_add", clear_on_submit=True):
        ...
        emis = c1.text_input("EMIS எண்", max_chars=13).strip()
        ...
        if st.form_submit_button("💾 சேமி"):
            if len(emis) == 13 and emis.isdigit() and name and s_class != "-- தேர்வு செய்க --":
                ...
                st.warning("13 இலக்க EMIS மற்றும் அனைத்து விபரங்களையும் அளிக்கவும்.")

# --- Tab 2: Bulk Upload (Excel/CSV) ---
with tab2:
    st.subheader("📤 எக்செல் அல்லது CSV கோப்பை பதிவேற்றவும்")
    st.info("கோப்பில் தலைப்புகள்: **emis_no, student_name, gender, class_name** என இருக்க வேண்டும்.")
...
if students_data:
    ...
    if search:
        final_df = final_df[final_df['student_name'].str.contains(search.upper()) | final_df['emis_no'].str.contains(search)]
    ...
    if sel_st != "-- தேர்வு செய்க --":
        ...
        with c_edit2:
            ...
            if st.button(f"❌ {s_emis} நீக்கு", type="primary"):

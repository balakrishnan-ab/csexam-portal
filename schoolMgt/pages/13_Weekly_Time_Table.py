...
st.set_page_config(page_title="Fixed Timetable Editor", layout="wide")
...
with main_col:
    # 🎯 செஷன் ஸ்டேட்டை ரீசெட் செய்ய ஒரு வழி
    selected_class = st.selectbox("வகுப்பைத் தேர்வு செய்க:", ["-- Select Class --"] + class_list, key="class_selector")
...
if selected_class != "-- Select Class --":
    ...
    # 1. ஆரம்ப கால தரவுத் தயாரிப்பு (வகுப்பு மாறும்போது மட்டும்)
    state_key = f"df_state_{selected_class}"
    ...
    with side_col:
        ...
        # ⚠️ மிக முக்கியம்: கம்பைன் வகுப்புகளையும் சேர்த்துத் தேடுதல்
        ...
        available_options = [""] # ஆரம்பத்தில் காலியாக
        
        if not class_staff:
            st.warning("இந்த வகுப்புக்கு ஆசிரியர்கள் ஒதுக்கப்படவில்லை!")
        else:
    ...
    with main_col:
        # 🎯 ஆப்ஷன்கள் இருப்பதை உறுதி செய்த பின் எடிட்டரைக் காட்டுதல்
        edited_df = st.data_editor(
            st.session_state[state_key],
            column_config={p: st.column_config.SelectboxColumn(p, options=available_options, width="small") for p in periods},
            use_container_width=True,
            num_rows="fixed",
            key=f"editor_v2_{selected_class}"
        )
        
        # மாற்றங்களை உடனுக்குடன் அப்டேட் செய்தல்

...
if selected_exam_label != "-- தேர்வு செய்க --":
    ...
    # 🔥 ஏற்கனவே உள்ள மேப்பிங் விவரங்களை எடுக்கும் பகுதி
    ...
    if existing_data:
        with st.expander("🔔 இந்தத் தேர்விற்கு ஏற்கனவே பதியப்பட்ட விவரங்கள்", expanded=False):
            df_exist = pd.DataFrame(existing_data)
            st.dataframe(df_exist[['exam_no', 'student_name', 'class_name', 'gender']].sort_values('exam_no'), use_container_width=True, hide_index=True)
            st.info("புதிதாக உருவாக்கினால் மேலுள்ள தரவுகள் மாற்றப்படும்.")

    if sel_classes:
        df_stu = pd.DataFrame(students_list)
        # பாலினத் தரவைச் சுத்தம் செய்தல்
            ...
            with st.expander(f"📍 {cls} - ஒதுக்கீடு", expanded=True):
                ...
                for _, row in f_students.iterrows():
                    all_new_mappings.append({
                        "exam_id": selected_exam_id, "emis_no": str(row['emis_no']), 
                        "exam_no": current_num, "class_name": cls, 
                        "student_name": row['student_name'], "gender": "Female"
                    })
                    current_num += 1
                ...
                for _, row in m_students.iterrows():
                    all_new_mappings.append({
                        "exam_id": selected_exam_id, "emis_no": str(row['emis_no']), 
                        "exam_no": current_num, "class_name": cls, 
                        "student_name": row['student_name'], "gender": "Male"
                    })
                    current_num += 1
        ...
        if all_new_mappings:
            ...
            if st.button("🚀 எண்களை உறுதி செய்து சேமி", use_container_width=True, type="primary"):
                try:
                    # பழைய தரவை நீக்கிவிட்டுப் புதியவற்றைச் சேர்த்தல்

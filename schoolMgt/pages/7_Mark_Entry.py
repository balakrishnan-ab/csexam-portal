...
if sel_exam_name != "-- தேர்வு செய்க --":
    ...
    if sel_class != "-- தேர்வு செய்க --":
        ...
        if sel_display_name != "-- தேர்வு செய்க --":
            ...
            # ⚙️ விரைவு உள்ளீடு பட்டன்கள்
            st.divider()
            f1, f2 = st.columns(2)
            if f1.checkbox(f"அனைவருக்கும் அகமதிப்பீடு {max_i} நிரப்புக", key=f"bi_{state_key}"):
                st.session_state[state_key]['Internal'] = max_i
            
            if max_p > 0 and f2.checkbox(f"அனைவருக்கும் செய்முறை {max_p} நிரப்புக", key=f"bp_{state_key}"):
            ...
            # ⚡ மொத்தம் கணக்கிடுதல் (பிழையில்லாமல்)
            df_temp = st.session_state[state_key]
            df_temp['Theory'] = df_temp['Theory'].fillna(0)
            df_temp['Internal'] = df_temp['Internal'].fillna(0)
            df_temp['Practical'] = df_temp['Practical'].fillna(0)
            
            df_temp['Total'] = df_temp['Theory'] + df_temp['Internal'] + df_temp['Practical']
            df_temp.loc[df_temp['Abs'] == True, ['Theory', 'Practical', 'Internal', 'Total']] = 0
            ...
            # ⚡ மாற்றங்களை Session State-க்கு கொண்டு செல்லுதல்
            ...
            if st.button("🚀 மதிப்பெண்களை உறுதி செய்து சேமி", use_container_width=True, type="primary"):
                ...
                for idx, r in final_df.iterrows():
                    final_list.append({
                        "exam_id": exam_id, "emis_no": r['EMIS'], 

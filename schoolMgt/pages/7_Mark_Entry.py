...
if sel_exam_name and sel_sub_name and sel_class:
    ...
    fill_int = col_f1.checkbox(f"அனைவருக்கும் முழு அகமதிப்பீடு ({max_int})", key="master_int")
    fill_prac = False
    if max_prac > 0:
        fill_prac = col_f2.checkbox(f"அனைவருக்கும் முழு செய்முறை ({max_prac})", key="master_prac")
    ...
    for i, s in enumerate(students):
        ...
        if is_abs:
            ...
            # Theory
            t = row[3].number_input("T", 0, max_theory, key=f"t_{i}", label_visibility="collapsed")
            
            ptr = 4
            p = 0
            # ⚡ செய்முறை - டிக் செய்திருந்தால் 'value' தானாக மாறும்
            if max_prac > 0:
                p_val = max_prac if fill_prac else 0
                p = row[ptr].number_input("P", 0, max_prac, value=p_val, key=f"p_{i}", label_visibility="collapsed")
                ptr += 1
            
            # ⚡ அகமதிப்பீடு - டிக் செய்திருந்தால் 'value' தானாக மாறும்
            i_val = max_int if fill_int else 0
            intn = row[ptr].number_input("I", 0, max_int, value=i_val, key=f"int_{i}", label_visibility="collapsed")

...
# --- 3. CSS ஸ்டைலிங் (Dashboard & Topper Cards) ---
st.markdown("""
    <style>
...
# --- 4. அடிப்படைத் தரவுகளைப் பெறுதல் ---
...
# --- 5. தேர்வு மற்றும் வகுப்புத் தெரிவு (Dynamic Selection) ---
col1, col2 = st.columns(2)
sel_exam_name = col1.selectbox("1. தேர்வு (Exam):", [e['exam_name'] for e in exams_data])

# தேர்ந்தெடுக்கப்பட்ட தேர்வுக்குரிய ID
exam_id = next((e['id'] for e in exams_data if e['exam_name'] == sel_exam_name), None)

if exam_id:
    # 'exam_mapping' அட்டவணையில் இந்தத் தேர்வுக்குரிய வகுப்புகளை மட்டும் எடுத்தல்
    mapped_data = supabase.table("exam_mapping").select("class_name").eq("exam_id", exam_id).execute().data
    
    if mapped_data:
        # 12-A1, 12-B போன்ற பெயர்களில் இருந்து '12' போன்ற அடிப்படை வகுப்பை எடுத்தல்
        mapped_classes = sorted(list(set([
            str(m['class_name']).split('-')[0].strip() for m in mapped_data if m['class_name']
        ])), key=lambda x: int(x) if x.isdigit() else x)
        
        sel_class_main = col2.selectbox("2. வகுப்பு (Class):", ["-- தேர்வு செய்க --"] + mapped_classes)
    else:
        st.error("இந்தத் தேர்வுக்கு இன்னும் மாணவர்கள் ஒதுக்கப்படவில்லை.")
        sel_class_main = "-- தேர்வு செய்க --"
else:
    sel_class_main = col2.selectbox("2. வகுப்பு (Class):", ["-- தேர்வு செய்க --"])

# --- 6. முக்கிய பகுப்பாய்வு லாஜிக் ---
if sel_exam_name and sel_class_main != "-- தேர்வு செய்க --":
    split_gender = st.toggle("🔍 ஆண் பெண் பிரித்து காட்டு", value=True)
    st.divider()

    # அந்த வகுப்பிற்கு கீழ் வரும் அனைத்துப் பிரிவுகளையும் கண்டறிதல் (e.g., 12-A, 12-B...)
    relevant_sections = sorted(list(set([
        m['class_name'] for m in mapped_data if str(m['class_name']).startswith(sel_class_main)
    ])))
    ...
    if studs_mapping:
        # அனைத்துப் பிரிவுகளுக்குமான பொதுவான பாடப்பார்வை
        g_list = []
        for sec in relevant_sections:
            ...
            if c_info:
                ...
                if g_info: g_list.extend([s.strip() for s in g_info['subjects'].split(',')])
        g_list = sorted(list(set(g_list)))
        ...
        for s in studs_mapping:
            ...
            for sn in g_list:
                ...
                if m:
                    ...
                    if is_abs is None: # null = EXEMPTED
        ...
        st.markdown(f"""
            <div class="metric-container">
                ...
                <div class="metric-card"><div class="stat-label">AVG</div><div class="stat-val" style="color:blue">{avg_overall}</div></div>
            </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("🏅 பாடவாரி முதல் மூன்று மற்றும் கடைசி இடங்கள்"):
            ...
            for i, sn in enumerate(g_list):
                ...
                with target_col:
                    ...
                    if sorted_m:
                        st.markdown(f"<div class='topper-card'>🔝 { ' / '.join([f'{sm['name']} - {sm['sec']} ({sm['mark']})' for sm in sorted_m[:3]]) }</div>", unsafe_allow_html=True)
                        st.markdown(f"<div class='topper-card' style='border-left-color:red;'>🔻 {sorted_m[-1]['name']} - {sorted_m[-1]['sec']} ({sorted_m[-1]['mark']})</div>", unsafe_allow_html=True)
        ...
        for _, r in df_sorted.iterrows():
            d_row = {"Rank": r["Rank"], "தேர்வு எண்": r["தேர்வு எண்"], "பெயர்": r['பெயர்'], "பிரிவு": r['பிரிவு'], "மொத்தம்": r['மொத்தம்'], "Fails": r['Fails'], "தோல்வி விவரம்": r['தோல்வி விவரம்']}
            for sn in g_list:
                v = r.get(sn)

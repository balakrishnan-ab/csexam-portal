...
st.set_page_config(page_title="Class-wise Analysis Report", layout="wide")

# ⚡ CSS வடிவமைப்பு
st.markdown("""
    <style>
    .stDataFrame td { font-weight: bold !important; font-size: 14px !important; white-index: pre !important; }
...
if sel_exam_name and sel_class != "-- தேர்வு செய்க --":
    ...
    if students and relevant_subjects:
        ...
        m[3].markdown(f'<div class="main-stat"><div class="stat-label">Fail</div><div class="stat-val">{stats["present"]["all"]-stats["pass"]["all"]}</div></div>', unsafe_allow_html=True)
        ...
        with e2:
            if len(absent_list) > 0:
                with st.expander(f"🚶 தேர்வுக்கே வராதவர்கள்: {len(absent_list)} பேர்"):
                    for item in absent_list: st.markdown(f'<div class="info-card critical-card" style="border-left-color:#ef4444; background-color:#fef2f2;">❌ {item}</div>', unsafe_allow_html=True)
        ...
        for sub in relevant_subjects:
            ...
            f_app, f_pas, m_app, m_pas = 0, 0, 0, 0
        ...
        def style_f(row):
            ...
            for i, col in enumerate(row.index):
                ...
                if col in [s['subject_name'] for s in relevant_subjects]:
                    if val == "ABS" or (isinstance(val, int) and val < 35): styles[i] = 'color: red'
        ...
        # --- 📉 தோல்வி அடைந்தவர்களின் விவரம் (0-க்கு மேல் இருந்தால் மட்டும்) ---
        st.divider()

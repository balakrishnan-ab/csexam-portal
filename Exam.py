...
# --- CSS வடிவமைப்பு: பள்ளி பெயர், கொண்டாட்டம் மற்றும் வினா பலகம் ---
st.markdown("""
    <style>
    ...
    .q-box { border: 1px solid #ddd; padding: 20px; border-radius: 10px; background: white; margin-bottom: 10px; min-height: 150px; }
...
# --- 2. தரவுத்தள செயல்பாடுகள் (SheetDB) ---

# முந்தைய முயற்சிகளைச் சரிபார்க்க
...
try:
    ...
    if st.session_state.page == 'login':
        ...
        with col2:
            ...
            if not df_raw.empty:
                ...
                num_q = st.number_input(f"புதிய வினாக்கள் எண்ணிக்கை (மீதம்: {len(rem)}):", 1, len(rem), min(len(rem), 25))
                
                if st.button("தேர்வைத் தொடங்கு ➡️", type="primary") and u_name:
                    att_count = get_attempt_count(u_name, sel_sub)
                    st.session_state.attempt_no = att_count
    ...
    # --- முடிவு பக்கம் ---
        ...
        if not st.session_state.get('score_saved', False):
            success = save_score(st.session_state.user_name, st.session_state.selected_std, st.session_state.selected_subject, correct, total, attempted, st.session_state.attempt_no)
            st.session_state.score_saved = True
            if success: st.success(f"வாழ்த்துக்கள் {st.session_state.user_name}! உங்களின் {st.session_state.attempt_no}-வது முயற்சி வெற்றிகரமாகச் சேமிக்கப்பட்டது. ✅")

        st.markdown(f'<div style="border:10px solid #1E88E5; padding:30px; text-align:center; background-color:white;"><h2>அரசு மேல்நிலைப்பள்ளி - தேவனாங்குறிச்சி</h2><hr><h1>சரியானவை: {correct} / {total}</h1><p>முயற்சி எண்: {st.session_state.attempt_no}</p></div>', unsafe_allow_html=True)
